...
```
Juniors and seniors in the housing lottery are ordered by accumulated credit hours first, with ties broken randomly (admin_housing_lottery.txt).
```
...
```
In Aldridge Hall, a wash costs $1.75 and is card only. (housing_aldridge_hall_laundry.txt and housing_aldridge_hall.txt)
```
...
```
In Aldridge Hall, a wash costs $1.75 and is card only. This information comes from `housing_aldridge_hall_laundry.txt` and `housing_aldridge_hall.txt`.
```
...
```
One wash in Aldridge Hall costs $1.75, and it is card only (housing_aldridge_hall_laundry.txt and housing_aldridge_hall.txt).
```
...
```
Group study rooms can be booked two weeks ahead in two-hour blocks, with a maximum of two blocks per person per week (study_group_rooms.txt).
```
